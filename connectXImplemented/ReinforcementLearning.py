...
class ReinforcementLearningAgent():
  def __init__(self):
    s_img = Input( shape=(1,64,64),name='s_img',dtype='float32')
    sprime_img = Input( shape=(1,64,64),name='sprime_img',dtype='float32')
    s_layers= x_voxel
    ...
    self.supervised_policy.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size, input_dim=(1,64,64)) )
    self.supervised_policy.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size) )
    self.supervised_policy.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size) )
    self.supervised_policy.add( Dense(num_actions,activation='softmax') )
    self.supervised_policy.compile(loss = 'categorical_cross_entropy',optimizer='adam')


    # define the value network
    self.value_network = Sequential() 
    self.value_network.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size, input_dim=(1,64,64)) )
    self.value_network.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size) )
    self.value_network.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size) )
    ...
    self.policy_network.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size, input_dim=(1,64,64)) )
    self.policy_network.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size) )
    self.policy_network.add( Convolution3D(nb_filters = 32,kernel_dim1=kernel_size,kernel_dim2=kernel_size) )
