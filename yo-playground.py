...
model.add(Activation("linear"))
model.compile(optimizer='adagrad',loss="mse",metrics=["mean_squared_error"])

data = np.random.random((NUM_EXAMPLES,3))
f_a = lambda x,y,z: x + 2*y + 3*z#np.multiply(x,y) + 3*z
f_b = lambda x,y,z: x + 2*y - 3*z#np.multiply(x,y) - 3*z
...
for i in xrange(NUM_VALIDATION_POINTS):
	...
	print "correct result: ", validation_labels_a[i], ", ", validation_labels_b[i]
	print "true result: ", predicted_labels[i]
